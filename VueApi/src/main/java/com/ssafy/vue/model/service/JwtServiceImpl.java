...
//	전달 받은 토큰이 제대로 생성된것인지 확인 하고 문제가 있다면 UnauthorizedException을 발생.
	@Override
	public boolean checkToken(String jwt) {  // jwt를 전달 받는다.
		try {
//			Json Web Signature? 서버에서 인증을 근거로 인증정보를 서버의 private key로 서명 한것을 토큰화 한것
//			setSigningKey : JWS 서명 검증을 위한  secret key 세팅
//			parseClaimsJws : 파싱하여 원본 jws 만들기
			Jws<Claims> claims = Jwts.parser().setSigningKey(this.generateKey()).parseClaimsJws(jwt);
			//Jws는 JWT 서명(JWS, JSON Web Signature)의 파싱된 표현을 나타내는 인터페이스임.
			// 만약 올바른 `secretKey`가 없거나 JWT서명이 유효하지 않은 경우 `parseClaimsJws()`가 예외를 발생시킨다.!!
			//입력받은 JWT가 서버에서 사용하는 secretKey와 일치해야만 해당 JWT를 올바르게 파싱하고 검증할 수 있습니다. 
			//서버와 클라이언트 간에는 secretKey의 일치가 필요하며, 이는 JWT의 유효성 검증에 중요한 요소입니다.
...
//			개발환경
	...
	// 요청 헤더에서 jwt 접근해서 페이로드 데이터를 반환하는 메서드
	@Override
